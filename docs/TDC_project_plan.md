...
| `profile_confidence` | float [0,1] | Confidence score based on source, samples, and ambiguity |
| `profile_warning` | string | Semicolon-separated warnings such as `synthetic_estimate` |
| `indecision_flag` | bool | High entropy / low concentration / neutral body flag |
...
FUNCTION simulate_intrabar_ticks(open, high, low, close, n_ticks, seed) -> ndarray
    - Select an unknown but valid high/low visitation order
    - Select interior anchor indices for both extremes
    - Build noisy bridge segments from open -> extreme -> extreme -> close
    - Clip all synthetic prices to [low, high]
    - Force anchors to exactly hit O/H/L/C
    - RETURN path
```
...
- Density/POC/value-area values can now be derived from real intrabar data or from a **synthetic** OHLC bridge estimate.
- Before using synthetic-derived features for prediction, compare them against real intrabar profiles for the same instruments and periods. Treat synthetic results as estimates and use exported confidence/warning fields in downstream models.
