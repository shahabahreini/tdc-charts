...
def fetch_yf_data(ticker: str, interval: str, period: str) -> pd.DataFrame:
    ...
    if df.empty:
        raise DataFetchError(f"Yahoo Finance returned an empty dataframe for {ticker}. Check the ticker symbol.")
...
def get_data(config: DataConfig, enable_volume_weighting: bool = False) -> pd.DataFrame:
    ...
    if enable_volume_weighting and 'Volume' not in df.columns:
        logger.warning(f"'Volume' column missing for {config.ticker}. Falling back to unweighted density.")
